...
create table if not exists ims.orderlines(orderlineId int primary key auto_increment, orderId int, itemId int, foreign key(orderId) references orders(orderId), foreign key(itemId) references items(itemId));